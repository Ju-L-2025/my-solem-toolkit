...
class SolemAPI:
    ...
    async def turn_on(self) -> None:
        """Turn on controller (enable watering). Protocol: 3105 a0 00 01 0000"""
    ...
    async def turn_off_permanent(self) -> None:
        """Disable watering permanently. Protocol: 3105 c0 00 00 0000"""
        command = struct.pack(">HBBBH", 0x3105, 0xC0, 0x00, 0x00, 0x0000)
        await self._write_and_commit(command)

    async def turn_off_x_days(self, days: int) -> None:
        """Disable watering for X days (max 15). Protocol: 3105 c0 00 XX 0000"""
    ...
    async def sprinkle_station_x_for_y_minutes(self, station: int, minutes: int) -> None:
        """Manually water a station for Y minutes. Protocol: 3105 12 XX 00 YYYY (YYYY=seconds)"""
        station = max(1, min(station, 16))
        minutes = max(1, min(minutes, 720))  # max 12h
        seconds = minutes * 60
    ...
    async def sprinkle_all_stations_for_y_minutes(self, minutes: int) -> None:
        """Manually water all stations for Y minutes each. Protocol: 3105 11 00 00 XXXX (XXXX=seconds)"""
        minutes = max(1, min(minutes, 720))  # max 12h
        seconds = minutes * 60
    ...
    async def run_program_x(self, program: int) -> None:
        """Run a controller program by id (1-3). Protocol: 3105 14 00 XX 0000"""
    ...
    async def stop_manual_sprinkle(self) -> None:
        """Stop any running manual watering session. Protocol: 3105 15 00 ff 0000"""
